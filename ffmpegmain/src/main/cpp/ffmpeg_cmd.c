...
void init(JNIEnv *env) {
    ff_env = env;
    err_count = 0;
    ff_class = (*env)->FindClass(env, "com/hft/ffmpeg/main/utils/FFmpegKit");
...
FFMPEG_FUNC(jint, setFontConfigEnvironmentVariable, jstring variable_name,
            jstring variable_value) {
    const char *variableNameString = (*env)->GetStringUTFChars(env, variable_name, 0);
    const char *variableValueString = (*env)->GetStringUTFChars(env, variable_value, 0);

    int rc = setenv(variableNameString, variableValueString, 1);

    (*env)->ReleaseStringUTFChars(env, variable_name, variableNameString);
    (*env)->ReleaseStringUTFChars(env, variable_value, variableValueString);
    return rc;
}
