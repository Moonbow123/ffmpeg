...
//        ffmpeg -i %s -vcodec libx264 -acodec libmp3lame %s
     ...
     * @param isVertical the layout of video, which could be horizontal or vertical
     * @param outputPath output file
     * @return join success or not
     */
    public static String[] multiVideo(String input1, String input2, String outputPath, boolean isVertical) {
        String multiVideo = "ffmpeg -i -i -filter_complex hstack";//hstack: horizontal
        if (isVertical) {//vstack: vertical
    ...
    public static String[] addSubtitleIntoVideo2(Context context,String inputPath, String subtitlePath, String outputPath) {
        String subtitleCmd = "ffmpeg -i -i -map 0:v -map 0:a -map 1:s -c copy";
        return insert(subtitleCmd.split(" "), 2, inputPath, 4, subtitlePath, outputPath);
    }
